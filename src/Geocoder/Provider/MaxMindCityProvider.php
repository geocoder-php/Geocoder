 ...
 */
class MaxMindCityProvider extends AbstractProvider implements ProviderInterface
{
    /**
     * @var string
     */
    ...
    public function getGeocodedData($address)
    {
        ...
        if (!filter_var($address, FILTER_VALIDATE_IP)) {
            throw new UnsupportedException('The MaxMindCityProvider does not support street addresses.');
        }

        // This API does not support IPv6
        if (filter_var($address, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
            throw new UnsupportedException('The MaxMindCityProvider does not support IPv6 addresses.');
        }
    ...
    {
        throw new UnsupportedException('The MaxMindCityProvider is not able to do reverse geocoding.');
    }
    ...
    protected function executeQuery($query)
    {
        ...
        } elseif ($data[5] == 'IP_NOT_FOUND') {
            throw new NoResultException('Could not retrieve informations for the ip address provided.');
        }

        return array_merge($this->getDefaults(), array(
            'country'     => $this->getCountryNameFromCountryCode($data[0]),
            'countryCode' => '' === $data[0] ? null : $data[0],
            'regionCode'  => '' === $data[1] ? null : $data[1],
            'city'        => '' === $data[2] ? null : $data[2],
            'latitude'    => '' === $data[3] ? null : $data[3],
            'longitude'   => '' === $data[4] ? null : $data[4],
        ));
    }
